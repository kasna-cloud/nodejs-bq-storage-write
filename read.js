...
async function main() {
  ...
  async function bigqueryStorageQuickstart() {
    ...
    const tableId = 'sample';

    const tableReference = `projects/${myProjectId}/datasets/${datasetId}/tables/${tableId}`;
     ...
     */
    const readOptions = {
      selectedFields: ['request', 'url', 'token'],
      rowRestriction: 'request = 1',
    };
        ...
        try {
          ...
          do {
            ...
            if (decodedData.value) {
              names.add(decodedData.value.url);
              console.log('Check our names', names);
